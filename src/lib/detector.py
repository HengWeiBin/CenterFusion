...
class Detector(object):
    ...
    def run(self, image_or_path_or_tensor, meta={}):
        ...
        if isinstance(image_or_path_or_tensor, np.ndarray):
            ...
            if pc_dep is not None:
                ...
                pc_dep = pc_dep.to(
                    self.opt.device, non_blocking=self.opt.non_block_test)
            pre_processed = True
        ...
        for scale in self.opt.test_scales:
            ...
            if not pre_processed:
                ...
                # prefetch testing
                images = torch.cat(tuple(img for img in pre_processed_images['images'][scale]), 0) #wayne pre_processed_images['images'][scale][0]
    ...
    def _transform_scale(self, image, scale=1):
        ...
        if self.opt.fix_short > 0:
            if height < width:
                ...
                inp_width = (
                    int(width / height * self.opt.fix_short) + 63) // 64 * 64
            else:
                inp_height = (
                    int(height / width * self.opt.fix_short) + 63) // 64 * 64
    ...
    def _get_additional_inputs(self, dets, meta, with_hm=True):
        ...
        for det in dets:
            if det['score'] < self.opt.pre_thresh:
                continue
            bbox = self._trans_bbox(
                det['bbox'], trans_input, inp_width, inp_height)
        ...
        if with_hm:
            ...
            if self.opt.flip_test:
                input_hm = np.concatenate(
                    (input_hm, input_hm[:, :, :, ::-1]), axis=0)
    ...
    def process(self, images, pre_images=None, pre_hms=None,
        ...
        with torch.no_grad():
            calib = torch.from_numpy(meta['calib']).float().to(
                images.device).squeeze(0)
            torch.cuda.synchronize()
    ...
    def show_results(self, debugger, image, results):
        ...
        for j in range(len(results)):
            if results[j]['score'] > self.opt.vis_thresh:
                ...
                if 'tracking' in item:
                    debugger.add_arrow(
                        item['ct'], item['tracking'], img_id='generic')
        ...
        if self.opt.debug == 4:
            debugger.save_all_imgs(
                self.opt.debug_dir, prefix='{}'.format(self.cnt))
        else:
