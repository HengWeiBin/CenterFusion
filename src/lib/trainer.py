...
class ModelWithLoss(torch.nn.Module):
    ...
    def forward(self, batch, phase):
        ...
        # run the first stage
        outputs = self.model(
            batch['image'], pc_hm=pc_hm, pc_dep=pc_dep, calib=calib)
...
class Trainer(object):
    ...
    def run_epoch(self, phase, epoch, data_loader):
        ...
        print(pbar_title_indent + '\t'.join([loss.split('_')[-1] for loss in avg_loss_stats.keys()]) + '    RAM')
        pbar = tqdm(data_loader, desc=phase)
        ...
        for iter_id, batch in enumerate(pbar):
            ...
            for l in avg_loss_stats:
                ...
                pbar_msg += f'{avg_loss_stats[l].avg:.2f}' + ' '*4
    ...
    def debug(self, batch, output, iter_id, dataset):
        ...
        for i in range(1):
            ...
            debugger.add_blend_img(
                img, gt, 'gt_hm', trans=self.opt.hm_transparency)
            ...
            if 'pre_img' in batch:
                pre_img = batch['pre_img'][i].detach(
            ...
            for k in range(len(dets['scores'][i])):
                if dets['scores'][i, k] > opt.vis_thresh:
                    debugger.add_coco_bbox(
                        dets['bboxes'][i, k] *
                        opt.down_ratio, dets['clses'][i, k],
                        dets['scores'][i, k], img_id='out_pred')

                    if 'ltrb_amodal' in opt.heads:
                        debugger.add_coco_bbox(
                            dets['bboxes_amodal'][i, k] *
                            opt.down_ratio, dets['clses'][i, k],
            ...
            for k in range(len(dets_gt['scores'][i])):
                if dets_gt['scores'][i][k] > opt.vis_thresh:
                    ...
                    debugger.add_coco_bbox(
                        dets_gt['bboxes'][i][k] *
                        opt.down_ratio, dets_gt['clses'][i][k],
