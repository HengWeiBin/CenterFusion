...
class GenericDataset(data.Dataset):
    ...
    def __init__(self, opt=None, split=None, ann_path=None, img_dir=None):
        ...
        if opt is not None and split is not None:
            ...
            self.enable_meta = True if (opt.run_dataset_eval and split in [
                                        "val", "mini_val", "test"]) or opt.eval else False
        ...
        if self.opt.tracking:
            ...
            if self.opt.same_aug_pre and frame_dist != 0:
                ...
                trans_output_pre = get_affine_transform(
                    c_pre, s_pre, rot, [self.opt.output_w, self.opt.output_h])
    ...
    def create_pc_pillars(self, img, img_info, pc_2d, pc_3d, inp_trans, out_trans):
        ...
        if self.opt.debug:
            ...
            for i, p in enumerate(pc_inp[:3, :].T):
                ...
                rect_tl_ori = (np.min(int(
                    pc_2d[0, i]-pill_wh_ori[0, i]/2), 0), np.min(int(pc_2d[1, i]-pill_wh_ori[1, i]), 0))
                rect_br_ori = (
    ...
    def _add_instance(
        ...
        if 'amodel_offset' in self.opt.heads:
            if 'amodel_center' in ann:
                amodel_center = affine_transform(
    ...
    def _add_hps(self, ret, k, ann, gt_det, trans_output, ct_int, bbox, h, w):
        ...
        if self.opt.simple_radius > 0:
            hp_radius = int(simple_radius(
                h, w, min_overlap=self.opt.simple_radius))
        else:
