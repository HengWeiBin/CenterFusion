...
class GenericDataset(data.Dataset):
  ...
  # Static variable below is used to load radar point cloud
  SENSOR_NAME = {
      7: 'RADAR_FRONT', 9: 'RADAR_FRONT_LEFT', 
      10: 'RADAR_FRONT_RIGHT', 11: 'RADAR_BACK_LEFT', 
      12: 'RADAR_BACK_RIGHT',  8: 'LIDAR_TOP', 
      1: 'CAM_FRONT', 2: 'CAM_FRONT_RIGHT', 
      3:'CAM_BACK_RIGHT', 4: 'CAM_BACK', 5: 'CAM_BACK_LEFT',
      6: 'CAM_FRONT_LEFT'}
  RADARS_FOR_CAMERA = {
  ...
  def __init__(self, opt=None, split=None, ann_path=None, img_dir=None):
    ...
    if opt is not None and split is not None:
      ...
      self.enable_meta = True if (opt.run_dataset_eval and split in ["val", "mini_val", "test", "val_sub0.1","val_top1000.json"]) or opt.eval else False

      self.nusc = NuScenes(
          version=self.SPLITS[split], dataroot=img_dir, verbose=False)
  ...
  def _load_pc_data(self, img, img_info, inp_trans, out_trans, flipped=0):
    ...
    # given pc_2d, pc_3d, transform point in org img coord to feature map , filter out outside points
    #  project 3dbox(1.5, 0.2, 0.2) to feature map as  
  ...
  def _transform_pc(self, pc_2d, trans, img_width, img_height, filter_out=True):
    ...
    # cv2.transform transform point coords directly
  ...
  def _process_pc(self, pc_2d, pc_3d, img, inp_trans, out_trans, img_info):    
    ...
    if len(self.opt.pc_feat_lvl) > 0:
      # transform point in org img coord to target img coord , filter out outside points 
    ...
    if self.opt.pc_roi_method == "pillars":
      # for each point, project its give 3dbox(1.5, 0.2, 0.2) to 2d box in feature map
    ...
    for i in range(pc_N-1, -1, -1):
      for feat in self.opt.pc_feat_lvl:
        # [feature map coord xy + depth ]  actually ! 
        point = pc_feat[:,i]
        depth = point[2]
        ct = np.array([point[0], point[1]])
        ct_int = ct.astype(np.int32)
        
        # TODO : code run from here !
        if self.opt.pc_roi_method == "pillars":
          wh = pillar_wh[:,i]
          
          #  point position and box area 
          #            ________
          #           \                  \
          #           \                  \
          #           \___.____\
  ...
  def create_pc_pillars(self, img, img_info, pc_2d, pc_3d, inp_trans, out_trans):
    ...
    pillar_dim = self.opt.pillar_dims #[1.5,0.2,0.2]
    ...
    for i, center in enumerate(pc_3d[:3,:].T):
      ...
      # transform 2d corners from org img to feature map
