...
class PrefetchDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if opt.pointcloud:
            assert len(
                opt.test_scales) == 1, "Multi-scale testing not supported with pointcloud."
...
def prefetch_test(opt):
    ...
    print(' '*8 + (' '*14).join([time.split('_')[-1] for time in avg_time_stats.keys()]))
    ...
    if opt.use_loaded_results:
        ...
        num_iters = 0
    for ind, (img_id, pre_processed_images) in enumerate(pbar):
        if ind >= num_iters:
            break
        ...
        for t in avg_time_stats:
            ...
            pbar_msg += f'{avg_time_stats[t].val:.3f}s ({avg_time_stats[t].avg:.3f}s)' + ' '*2
